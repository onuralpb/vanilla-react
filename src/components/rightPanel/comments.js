...
import { Modal, Pagination, Row, Col } from 'react-bootstrap';
...
export default class comments extends Component {
	...
	componentDidMount() {
		...
		fetch(`http://localhost:5555/comments`).then(data => data.json()).then(data => {
			this.setState({
				totalPage :
		...
		setTimeout(() => {
			fetch(`http://localhost:5555/comments?_page=${currentPage}&_limit=${pageLimit}`)
				.then(data => data.json())
				.then(comments => {
					this.setState({
						comments  : comments,
	...
	handleDelete = userId => {
		fetch(`http://localhost:5555/comments/${userId}`, { method: 'DELETE' })
			.then(data => data.json())
			.then(() => {
	...
	handleSave = (e, userId) => {
		e.preventDefault();
		e.persist();
		fetch(`http://localhost:5555/comments/${userId}`, {
			method  : 'PUT',
			headers : {
	...
	getUpdateDataList = () => {
		...
		setTimeout(() => {
			let URI = `http://localhost:5555/comments?_page=${this.state.currentPage}&_limit=${this.state.pageLimit}`;
			console.log(URI);
